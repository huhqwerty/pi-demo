...
interface WindowWithEnv extends Window {
  __ENV?: {
    ...
    platform_api_url: string,   // REACT_APP_PLATFORM_API_URL environment variable
    pi_api_key: string,         // REACT_APP_PI_API_KEY environment variable
  }
}

const _window: WindowWithEnv = window;
const backendURL = _window.__ENV && _window.__ENV.backendURL;
const platform_api_url = _window.__ENV && _window.__ENV.platform_api_url;
const pi_api_key = _window.__ENV && _window.__ENV.pi_api_key;
...
export default function Shop() {
  ...
  const onIncompletePaymentFound = (payment: PaymentDTO) => {
    ...
    return axiosClient.post('/payments/incomplete', {platformAPIClient, payment});
  }

  const onReadyForServerApproval = (paymentId: string) => {
    console.log("onReadyForServerApproval", paymentId);
    axiosClient.post('/payments/approve', {platformAPIClient, paymentId}, config);
  }

  const onReadyForServerCompletion = (paymentId: string, txid: string) => {
    console.log("onReadyForServerCompletion", paymentId, txid);
    axiosClient.post('/payments/complete', {platformAPIClient, paymentId, txid}, config);
  }

  const onCancel = (paymentId: string) => {
    console.log("onCancel", paymentId);
    return axiosClient.post('/payments/cancelled_payment', {platformAPIClient, paymentId});
  }
